...
int x=680,y=250,bg_x=-800,heart=5,black_heart=0,score=0,n=2;
...
void draw(){
  if(unstart& !stop){
    ...
    //碰撞敵人偵測
    for(int i=0;i<n;i++){
      if(dist(x, y+50, enemy_x[i]+35, enemy_y[i]+40)<=55){
        heart-=1;
        black_heart+=1;
        enemy_x[i]=0;
        enemy_y[i]=0;
    ...
    for(int i=0;i<3;i++){
      if(dist(x,y+50,coin_x[i]+30,coin_y[i]+30)<=35){
        coin_x[i]=0;
        coin_y[i]=0;
        score+=150;
        add=true;
...
void set_enemy_xy(){
  ...
  for (int i = 0; i < n; i++) {
    // 如果敵人超出畫布底部，重新設定位置
    if (enemy_x[i] >=width | (enemy_x[i]==0 & enemy_y[i]==0)) {
      enemy_y[i] = random(0,height);
      enemy_x[i] = random(-width,0);
    }else{
...
void coin(){
  for (int i=0;i<3;i++){
    if(coin_x[i]>=width | (coin_x[i]==0 &coin_y[i]==0)){
